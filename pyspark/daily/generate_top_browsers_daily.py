...
def generate_top_browsers_daily(process_date, input_path, output_path):
    spark = SparkSession.builder \
        .appName(f"TopBrowsersDaily_for_{process_date}") \
    ...
    df_result = df.withColumn("log_time", to_date("log_time")) \
        .groupBy(to_date("process_time").alias("process_time"), "log_time", "browser") \
        .agg(count("*").alias("count"))

    df_result.write.format("delta").mode("overwrite") \
        .option("replaceWhere", f"process_time = '{process_date}'").save(output_path)

    print(f"Generated top_browsers_daily for {process_date}")
    spark.stop()
...
if __name__ == "__main__":
    ...
    generate_top_browsers_daily(args.process_date, args.input_path, args.output_path)